...
import { ListTodo, Loader2, CheckCircle2, ArrowRight } from "@/components/ui/icon";
import { getLabDashboardData } from "@/actions/dashboards";

type Data = Extract<Awaited<ReturnType<typeof getLabDashboardData>>, { ok: true }>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Pending Tests" value={data.stats.pendingTests} icon={ListTodo} accent="#f59e0b" />
        <StatCard label="In Progress" value={data.stats.inProgress} icon={Loader2} accent="#3b82f6" />
        <StatCard label="Completed Today" value={data.stats.completedToday} icon={CheckCircle2} accent="#22c55e" />
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {data.testQueue.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No tests in queue</p>
          ) : (
                ...
                <tbody>
                  {data.testQueue.map((o) => (
                    <tr key={o.id} className="border-b border-border/50">
                      ...
                      <td className="py-3 pr-2 text-foreground">{o.testName}</td>
                      <td className="py-3 pr-2">{priorityBadge(o.priority)}</td>
                      <td className="py-3 pr-2 text-xs text-muted-foreground">{formatTime(o.createdAt)}</td>
                      <td className="py-3 text-right">
                        <Link href={`/lab/results/${o.id}`}>
                          <Button size="sm" variant="outline">
                            {o.status === "ORDERED" ? "Collect Sample" : "Enter Results"}
                          </Button>
                        </Link>
                      </td>
                    </tr>
